@extends('layouts.app')
@section('title', 'Cashless Facility – Bilal Hospital Mumbra')
@section('meta_description', 'Bilal Hospital is empanelled with 30+ insurance companies and TPAs for cashless treatment in Mumbra, Thane.')
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div style="text-align:center;position:relative;" data-aos="fade-up" data-aos-delay="{{ $i * 100 }}">
                @if($i < 3)
                ...
                <div style="width:60px;height:60px;background:#1B3A7A;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;position:relative;z-index:1;">
                    <i class="fas {{ $step['icon'] }}" style="color:#fff;font-size:22px;"></i>
                </div>
                <div style="display:inline-block;background:#F5821F;color:#fff;font-family:Montserrat;font-weight:700;font-size:11px;padding:4px 12px;border-radius:20px;margin-bottom:10px;">STEP {{ $i + 1 }}</div>
                <h4 style="font-family:Montserrat;font-weight:700;color:#1B3A7A;font-size:14px;margin-bottom:8px;">{{ $step['title'] }}</h4>
                <p style="font-family:Ubuntu;font-size:13px;color:#6b6b6b;line-height:1.7;">{{ $step['desc'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
