@extends('layouts.app')
@section('title', 'Contact Us – Bilal Hospital Mumbra')
@section('meta_description', 'Contact Bilal Hospital, Mumbra. OPD: 9699911101. Address: Royal Garden, Old Mumbai-Pune Road, Mumbra, Thane 400612.')
                ...
                <div class="space-y-5">
                    ...
                    <div style="display:flex;gap:16px;align-items:flex-start;">
                        <div style="width:48px;height:48px;background:rgba(27,58,122,0.08);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            <i class="fas {{ $item['icon'] }}" style="color:#1B3A7A;font-size:18px;"></i>
                        </div>
                        <div>
                            <p style="font-family:Montserrat;font-weight:700;font-size:12px;color:#1B3A7A;margin-bottom:4px;letter-spacing:0.5px;text-transform:uppercase;">{{ $item['label'] }}</p>
                            @if($item['link'])
                            <a href="{{ $item['link'] }}" style="font-family:Ubuntu;font-size:14px;color:#6b6b6b;text-decoration:none;">{{ $item['value'] }}</a>
                            @else
                            <p style="font-family:Ubuntu;font-size:14px;color:#6b6b6b;">{{ $item['value'] }}</p>
                            @endif
                        </div>
                    </div>
                    @endforeach
