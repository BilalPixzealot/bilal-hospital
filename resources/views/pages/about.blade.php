@extends('layouts.app')
@section('title','About Bilal Hospital – Multispecialty Hospital Mumbra')
@section('meta_description','Learn about Bilal Hospital – a 50-bedded multispecialty hospital in Mumbra, Thane. ISO 9001:2015 Certified & NABH Pre-Accredited with 17+ years of healthcare excellence.')
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            @endphp
            @foreach($mvv as $item)
            <div style="background:#fff;border-radius:12px;padding:36px 28px;box-shadow:0 2px 20px rgba(27,58,122,0.07);border-top:4px solid {{ $item['color'] }};" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <div style="width:52px;height:52px;background:rgba(245,130,31,0.1);border-radius:10px;display:flex;align-items:center;justify-content:center;margin-bottom:20px;">
                    <i class="fas {{ $item['icon'] }}" style="color:{{ $item['color'] }};font-size:22px;"></i>
                </div>
                <h3 style="font-family:Montserrat;font-weight:800;font-size:18px;color:#1B3A7A;margin-bottom:14px;">{{ $item['title'] }}</h3>
                <p style="font-family:Ubuntu;color:#6b6b6b;font-size:14px;line-height:1.8;">{{ $item['text'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
            ...
            <div style="background:#F7F9FC;border-radius:10px;padding:28px 20px;text-align:center;transition:all 0.3s;" class="hover:shadow-md hover:-translate-y-1" data-aos="fade-up" data-aos-delay="{{ $loop->index * 50 }}">
                <div style="width:52px;height:52px;background:rgba(27,58,122,0.08);border-radius:10px;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;">
                    <i class="fas {{ $item['icon'] }}" style="color:#1B3A7A;font-size:22px;"></i>
                </div>
                <p style="font-family:Montserrat;font-weight:700;font-size:13px;color:#1B3A7A;">{{ $item['title'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section style="background:linear-gradient(135deg,#1B3A7A 0%,#122550 100%);padding:80px 0;">
    <div class="max-w-7xl mx-auto px-5">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div style="background:rgba(255,255,255,0.07);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.15);border-radius:12px;padding:32px 16px;text-align:center;" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
...
<section style="background:#F5821F;padding:80px 0;">
    <div class="max-w-7xl mx-auto px-5 text-center" data-aos="fade-up">
        ...
        <div class="flex flex-wrap gap-4 justify-center">
            <a href="{{ route('appointment') }}" class="btn-outline"><i class="fas fa-calendar-check"></i> Book Appointment</a>
