...
@section('title', 'Bilal Hospital – Multispecialty Hospital in Mumbra, Thane')
@section('meta_description', 'Bilal Hospital is a 50-bedded multispecialty hospital in Mumbra, Thane with well-equipped ICU, NICU, OT and diagnostic centre. ISO 9001:2015 Certified | NABH Pre-Accredited.')
...
<section class="hero-section relative overflow-hidden">
    ...
    <div class="hero-slide {{ $i === 0 ? 'active' : '' }}">
        <img src="{{ $slide['bg'] }}" alt="Bilal Hospital" class="hero-bg" {{ $i === 0 ? 'fetchpriority="high"' : 'loading="lazy"' }} width="1600" height="900">
        <div class="hero-overlay-dark"></div>
        <div class="hero-content relative z-10 w-full max-w-7xl mx-auto px-6 pt-24">
            <p style="font-family:Montserrat;font-weight:700;font-size:11px;letter-spacing:4px;color:#F5821F;text-transform:uppercase;margin-bottom:16px;">{{ $slide['tag'] }}</p>
            <h1 style="font-family:Montserrat;font-weight:900;font-size:clamp(2.6rem,6vw,5rem);color:#fff;line-height:1.1;margin-bottom:20px;white-space:pre-line;">{{ $slide['heading'] }}</h1>
            <div style="width:60px;height:4px;background:#F5821F;border-radius:2px;margin-bottom:24px;"></div>
            <p style="font-family:Ubuntu,sans-serif;font-size:16px;color:rgba(255,255,255,0.82);max-width:600px;margin-bottom:36px;line-height:1.7;">{{ $slide['sub'] }}</p>
    ...
    <div class="max-w-7xl mx-auto px-5">
        <div class="grid grid-cols-2 lg:grid-cols-4 divide-x divide-white/10">
            ...
            <div class="flex items-center gap-4 py-5 px-6">
                <div class="info-icon"><i class="fas {{ $info['icon'] }} text-accent text-lg"></i></div>
                <div>
                    <p style="font-family:Montserrat;font-size:10px;font-weight:700;letter-spacing:2px;color:rgba(255,255,255,0.5);text-transform:uppercase;">{{ $info['label'] }}</p>
                    @isset($info['href'])
                    <a href="{{ $info['href'] }}" style="font-family:Ubuntu;font-size:14px;color:#fff;font-weight:500;">{{ $info['val'] }}</a>
                    @else
                    <p style="font-family:Ubuntu;font-size:14px;color:#fff;font-weight:500;">{{ $info['val'] }}</p>
                    @endisset
                </div>
            </div>
            @endforeach
    ...
    <div class="max-w-7xl mx-auto px-5">
        <div class="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div data-aos="fade-left">
                ...
                <a href="{{ route('about') }}" class="btn-navy">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="stat-card" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <div style="font-family:Montserrat;font-weight:900;font-size:clamp(2rem,4vw,3rem);color:#fff;line-height:1;margin-bottom:8px;">
                    <span class="counter-num" data-target="{{ $stat->value }}">0</span><span style="color:#F5821F;">{{ $stat->suffix }}</span>
                </div>
                <div style="font-family:Ubuntu;font-size:13px;color:rgba(255,255,255,0.6);text-transform:uppercase;letter-spacing:1px;">{{ $stat->label }}</div>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <a href="{{ route('departments.show', $dept->slug) }}" class="dept-grid-card" data-aos="fade-up" data-aos-delay="{{ ($loop->index % 4) * 80 }}">
                <div class="dept-icon-box">
                    <i class="fas fa-stethoscope text-primary text-xl"></i>
                </div>
                <h3 style="font-family:Montserrat;font-weight:700;color:#1B3A7A;font-size:14px;margin-bottom:8px;">{{ $dept->name }}</h3>
                @if($dept->short_description)
                <p style="font-family:Ubuntu;font-size:13px;color:#6b6b6b;line-height:1.6;margin-bottom:12px;" class="line-clamp-2">{{ $dept->short_description }}</p>
                @endif
        ...
        <div class="text-center mt-12" data-aos="fade-up">
            <a href="{{ route('departments') }}" class="btn-navy">
...
<section class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <a href="{{ route('doctors.show', $doctor->slug) }}" class="doc-card" data-aos="fade-up" data-aos-delay="{{ ($loop->index % 4) * 80 }}">
                <div class="doc-img">
                    @if($doctor->photo)
                    ...
                    @endif
                    <div class="doc-tag">
                        <p style="font-family:Montserrat;font-size:11px;font-weight:700;color:rgba(255,255,255,0.7);letter-spacing:0.5px;">{{ $doctor->designation }}</p>
                    </div>
                </div>
                <div style="padding:16px;">
                    <h3 style="font-family:Montserrat;font-weight:700;font-size:14px;color:#1B3A7A;margin-bottom:4px;">{{ $doctor->name }}</h3>
                    <p style="font-family:Ubuntu;font-size:12px;color:#F5821F;font-weight:500;">{{ Str::limit($doctor->qualifications, 50) }}</p>
                    @if($doctor->department)
                    <p style="font-family:Ubuntu;font-size:12px;color:#6b6b6b;margin-top:4px;">{{ $doctor->department->name }}</p>
                    @endif
                </div>
            </a>
            @endforeach
        ...
        <div class="text-center mt-12" data-aos="fade-up">
            <a href="{{ route('doctors') }}" class="btn-primary">
    ...
    <div class="max-w-7xl mx-auto px-5">
        <div class="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5" data-aos="fade-left">
                ...
                <div style="background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.12);border-radius:10px;padding:20px;" data-aos="fade-up" data-aos-delay="{{ $loop->index * 60 }}">
                    <div style="width:42px;height:42px;background:rgba(245,130,31,0.15);border-radius:8px;display:flex;align-items:center;justify-content:center;margin-bottom:12px;">
                        <i class="fas {{ $r['icon'] }}" style="color:#F5821F;font-size:18px;"></i>
                    </div>
                    <h4 style="font-family:Montserrat;font-weight:700;font-size:13px;color:#fff;margin-bottom:6px;">{{ $r['title'] }}</h4>
                    <p style="font-family:Ubuntu;font-size:12px;color:rgba(255,255,255,0.55);line-height:1.6;">{{ $r['desc'] }}</p>
                </div>
                @endforeach
            </div>
        </div>
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
            ...
            <div class="bg-white rounded-xl p-5 text-center shadow-sm hover:shadow-md transition-all" data-aos="fade-up" data-aos-delay="{{ $loop->index * 40 }}">
                <div style="width:48px;height:48px;background:rgba(27,58,122,0.08);border-radius:10px;display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
                    <i class="fas {{ $svc['icon'] }}" style="color:#1B3A7A;font-size:20px;"></i>
                </div>
                <p style="font-family:Montserrat;font-weight:700;font-size:12px;color:#1B3A7A;">{{ $svc['name'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
<section class="py-24 bg-white">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="testi-card" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <div class="flex gap-1 mb-4">
                    @for($i = 0; $i < $t->rating; $i++)<i class="fas fa-star" style="color:#F5821F;font-size:13px;"></i>@endfor
                </div>
                <p style="font-family:Ubuntu;font-size:15px;color:#4b5563;line-height:1.8;margin-bottom:20px;position:relative;z-index:1;">"{{ $t->review }}"</p>
                <div class="flex items-center gap-3">
                    <div style="width:42px;height:42px;background:#1B3A7A;border-radius:50%;display:flex;align-items:center;justify-content:center;">
                        <span style="font-family:Montserrat;font-weight:700;color:#fff;font-size:15px;">{{ substr($t->patient_name,0,1) }}</span>
                    </div>
                    <div>
                        <p style="font-family:Montserrat;font-weight:700;font-size:13px;color:#1B3A7A;">{{ $t->patient_name }}</p>
                        <p style="font-family:Ubuntu;font-size:12px;color:#6b6b6b;">{{ $t->source }} Review</p>
                    </div>
                    @if($t->source === 'Google')
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div class="gallery-item" data-aos="fade-up" data-aos-delay="{{ $loop->index * 60 }}">
                <img src="{{ asset('storage/' . $img->image) }}" alt="{{ $img->title ?? 'Hospital' }}" loading="lazy" width="480" height="300"
                     style="width:100%;height:220px;object-fit:cover;display:block;">
            </div>
            @endforeach
        </div>
        <div class="text-center mt-10" data-aos="fade-up">
            <a href="{{ route('gallery') }}" class="btn-navy">View Full Gallery <i class="fas fa-arrow-right"></i></a>
        </div>
    </div>
</section>
@endif
...
<section class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-5">
        <div style="background:linear-gradient(135deg,#1B3A7A 0%,#122550 100%);border-radius:16px;padding:56px 48px;display:flex;flex-wrap:wrap;align-items:center;gap:32px;justify-content:space-between;" data-aos="fade-up">
            ...
            <a href="{{ route('cashless') }}" class="btn-primary">
...
<section style="background:#F5821F;padding:80px 0;">
    <div class="max-w-7xl mx-auto px-5 text-center" data-aos="fade-up">
        ...
        <div class="flex flex-wrap gap-4 justify-center">
            <a href="{{ route('appointment') }}" class="btn-outline">
...
<section class="py-24" style="background:#F7F9FC;">
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <article class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-1 transition-all" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                @if($post->featured_image)
                <img src="{{ asset('storage/'.$post->featured_image) }}" alt="{{ $post->title }}" loading="lazy" width="480" height="220" style="width:100%;height:200px;object-fit:cover;display:block;">
                @else
                ...
                <div style="padding:24px;">
                    @if($post->category)
                    <span style="background:#FFF7ED;color:#F5821F;font-family:Montserrat;font-weight:700;font-size:11px;letter-spacing:1px;padding:4px 12px;border-radius:4px;text-transform:uppercase;">{{ $post->category }}</span>
                    @endif
                    <h3 style="font-family:Montserrat;font-weight:700;font-size:16px;color:#1B3A7A;margin:12px 0 8px;line-height:1.4;">{{ $post->title }}</h3>
                    <p style="font-family:Ubuntu;font-size:13px;color:#6b6b6b;line-height:1.7;margin-bottom:16px;" class="line-clamp-3">{{ $post->excerpt }}</p>
                    <a href="{{ route('blog.show', $post->slug) }}" style="font-family:Montserrat;font-weight:700;font-size:12px;color:#F5821F;letter-spacing:0.5px;text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
        ...
        <div class="text-center mt-10" data-aos="fade-up">
            <a href="{{ route('blog') }}" class="btn-navy">View All Articles <i class="fas fa-arrow-right"></i></a>
        </div>
    </div>
</section>
@endif
    ...
    <div class="max-w-7xl mx-auto px-5">
        ...
        <div class="grid lg:grid-cols-5 gap-10 items-start">
            ...
            <div class="lg:col-span-2 space-y-6" data-aos="fade-left">
                ...
                <div style="display:flex;gap:16px;align-items:flex-start;" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">
                    <div style="width:44px;height:44px;background:rgba(27,58,122,0.08);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="fas {{ $c['icon'] }}" style="color:#1B3A7A;font-size:18px;"></i>
                    </div>
                    <div>
                        <p style="font-family:Montserrat;font-weight:700;font-size:12px;letter-spacing:1px;color:#1B3A7A;text-transform:uppercase;margin-bottom:4px;">{{ $c['title'] }}</p>
                        <p style="font-family:Ubuntu;font-size:14px;color:#6b6b6b;line-height:1.6;">{!! $c['content'] !!}</p>
                    </div>
                </div>
                @endforeach
                <a href="{{ route('contact') }}" class="btn-primary mt-4 inline-flex">
